...
class FrontController extends Controller
{
    ...
    public function package($slug)
    {
        ...
        $package_amenities_includes = PackageAmenity::with('amenity')->where('package_id', $package->id)->where('type', 'include')->get();
        $package_amenities_excludes = PackageAmenity::with('amenity')->where('package_id', $package->id)->where('type', 'exclude')->get();
    ...
    public function payment(Request $request)
    {
        ...
        if ($total_allowed_seats != '-1') {
            ...
            if ($total_booked_seats + $request->total_person > $total_allowed_seats) {
                return redirect()
                    ->back()
                    ->with('error', 'Sorry! Only ' . $total_allowed_seats . 'seats are available for this tour!');
            }
